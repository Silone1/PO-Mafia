...
*/
        ...
        function arrayChecker(object, array, sel) {
            return function (event) {
                ...
                if (warn_roles.length != 0) {
                    ...
                    $("#dialog").dialog("option", "title", "Unknown roles").dialog("option", "buttons", {
                        Ok: function () {
                            $(this).dialog("close")
                        }
                    }).dialog("open");
        ...
        $("#center-content select[name=distract_select]").change(function (event) {
            if (event.currentTarget.value == "ignore" || event.currentTarget.value == "ChangeTarget") {
                ...
                $("button[name=action-distract-set-roles]").removeAttr("disabled").unbind('click').bind('click', function (event) {
                    $("#dialog").dialog("option", "title", "Roles to ignore when they are distracting").dialog("option", "buttons", {
                        Ok: arrayChecker(role.actions.distract.mode, "killif"),
                        Cancel: function () {
                            $(this).dialog('close')
                        }
                    }).dialog("open");
                ...
                $("button[name=action-distract-set-roles]").removeAttr("disabled").unbind('click').bind('click', function (event) {
                    ...
                    $("#dialog").dialog("option", "title", "Roles to kill when they are distracting").dialog("option", "buttons", {
                        Ok: arrayChecker(role.actions.distract.mode, "ignore"),
                        Cancel: function () {
                            $(this).dialog('close')
                        }
                    }).dialog("open");
    ...
    function buildEditRoles(event) {
        ...
        roles.forEach(function (role1) {
            ...
            select_elem.change(function (i1, i2) {
                return function (event) {
                    roles[i1] = theme.roles[event.target.value].role;
                }
            }(index1 - 1));
    ...
    function buildUI(theme) {
        ...
        /* Resize accordion */
        $("#accordion").accordion().resizable(); /* Display global options */
        buildGlobal();
    ...
    $(document).ready(function () {
        ...
        $("#accordion").accordion({
            ...
            change: function (event, ui) {
                ...
                if (ui.newHeader.text() == "Theme globals") buildGlobal();
                else $("#center-content").html('');
            }
